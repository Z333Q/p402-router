// Layer 1 shape test for v2_058_access_requests_intent_columns.sql.
...
const UP_PATH = join(ROOT, 'scripts/migrations/v2_058_access_requests_intent_columns.sql');
const DOWN_PATH = join(ROOT, 'scripts/migrations/v2_058_access_requests_intent_columns_down.sql');
...
const ADDED_COLUMNS = ['intent', 'resolved_intent', 'plan_id', 'offer_id'] as const;

describe('v2_058_access_requests_intent_columns — up migration shape', () => {
    ...
    it('only ALTERs access_requests', () => {
        const alters = UP.match(/\bALTER\s+TABLE\s+([a-zA-Z_]\w*)/gi) ?? [];
        expect(alters.length).toBeGreaterThan(0);
        for (const stmt of alters) {
            expect(stmt.toLowerCase()).toMatch(/\balter\s+table\s+access_requests\b/);
        }
    });

    it('adds exactly the four planned columns as TEXT', () => {
        for (const col of ADDED_COLUMNS) {
            const re = new RegExp(`ADD\\s+COLUMN\\s+IF\\s+NOT\\s+EXISTS\\s+${col}\\s+TEXT\\b`, 'i');
            expect(UP, `missing ADD COLUMN for ${col}`).toMatch(re);
        }
    });

    it('declares no NOT NULL on the added columns', () => {
        const block = UP.match(/ALTER\s+TABLE\s+access_requests[\s\S]*?;/i);
    ...
    it('does not touch tenants or billing tables', () => {
        const ddl = UP
            .split('\n')
            .filter((l) => !l.trim().startsWith('--'))
            .join('\n');
        expect(ddl).not.toMatch(/\btenants\b/i);
...
describe('v2_058_access_requests_intent_columns_down — down migration shape', () => {
    ...
    it('drops exactly the four added columns, idempotently', () => {
        for (const col of ADDED_COLUMNS) {

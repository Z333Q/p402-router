...
- No backfill, ever. Existing tenants stay NULL; `/dashboard` gate (§4.6) treats NULL as "send to onboarding once." See §13 Q8 — this decision is final: pre-launch / test tenants flow through the new onboarding so the V5 funnel is validated on real navigation.
...
8. **Existing tenants `onboarded_at` backfill.** **Resolved (operator decision after 3AZ-2-E): no backfill.** There are no real customers yet; existing tenants are pre-launch / test accounts. They should be routed through the new onboarding flow again on their next dashboard visit so we can validate the V5 funnel on real (if internal) users. The 3AZ-2-B gate already produces this behavior: any tenant with `onboarded_at IS NULL` is redirected from `/dashboard` to `/onboarding/welcome`. No session invalidation is required — the gate sits in front of every dashboard render, so logged-in users encounter it on their next page load. The draft `v2_061_tenants_onboarded_at_backfill.sql` (and its down + shape-test files) were deleted; no backfill migration ships.

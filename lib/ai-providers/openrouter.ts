...
import { GENERATED_OPENROUTER_CATALOG } from './openrouter-catalog.generated';
...
export class OpenRouterAdapter extends BaseProviderAdapter {
    ...
    constructor(config: OpenRouterAdapterConfig = {}) {
        ...
        // Prefer the build-time live snapshot when present. Closes the
        // cold-start window where the very first request after a fresh
        // serverless instance would otherwise use the hand-curated static
        // list (which may carry retired ids). Generated by
        // scripts/build-openrouter-catalog.ts during prebuild; empty
        // until that script runs successfully.
        if (GENERATED_OPENROUTER_CATALOG.length > 0) {
            this.models = GENERATED_OPENROUTER_CATALOG;
        }
    }

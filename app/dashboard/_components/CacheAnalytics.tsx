...
export function CacheAnalytics() {
    ...
    return (
        <Card title="SEMANTIC CACHE">
            ...
            {/* Cache Stats */}
            <div className="grid grid-cols-3 gap-4 py-4 border-y-2 border-black mb-6">
                <div className="text-center">
                    <span className="text-2xl font-extrabold font-mono block">
                        {data?.totalEntries?.toLocaleString() || 0}
                    </span>
                    <span className="text-xs text-neutral-500 uppercase">Entries</span>
                </div>
                <div className="text-center">
                    <span className="text-2xl font-extrabold font-mono block">
                        {data?.totalHits?.toLocaleString() || 0}
                    </span>
                    <span className="text-xs text-neutral-500 uppercase">Total Hits</span>
                </div>
                <div className="text-center">
                    <span className="text-2xl font-extrabold font-mono block">
                        {(data?.avgHitsPerEntry || 0).toFixed(1)}
                    </span>
                    <span className="text-xs text-neutral-500 uppercase">Avg Hits</span>
                </div>
            </div>

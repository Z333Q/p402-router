...
function ProviderCard({ provider }: { provider: Provider }) {
    ...
    return (
        <div
            className={`border-2 ${statusColors[health.status]} p-3 bg-white hover:translate-y-[-2px] transition-all cursor-pointer group ${expanded ? 'col-span-full' : ''}`}
        ...
        >
            ...
            <div className="flex justify-between items-center text-xs text-neutral-500 mb-2">
                ...
                <span className="text-neutral-300 group-hover:text-primary transition-colors">{expanded ? '▲' : '▼'}</span>
            </div>

            {health.status !== 'unknown' && !expanded && (
            ...
            {expanded && (
                <div className="mt-4 pt-4 border-t-2 border-black overflow-x-auto">
                    <table className="w-full text-[10px] text-left">
                        <thead className="text-neutral-500 uppercase">
                            <tr>
                                ...
                                <th className="pb-2 text-right">In $/1K</th>
                                <th className="pb-2 text-right">Out $/1K</th>
                            </tr>
                        </thead>
                        <tbody>
                            {provider.models.map(model => (
                                <tr key={model.id} className="border-t border-neutral-100">
                                    <td className="py-2 font-mono">{model.id}</td>
                                    <td className="py-2">
                                        <Badge variant={model.tier === 'premium' ? 'primary' : model.tier === 'budget' ? 'success' : 'default'} className="!text-[8px] !px-1">
                                            {model.tier}
                                        </Badge>
                                    </td>
                                    <td className="py-2 font-mono">{(model.context_window / 1000).toFixed(0)}K</td>
                                    <td className="py-2 font-mono text-right">${model.input_cost_per_1k.toFixed(5)}</td>
                                    <td className="py-2 font-mono text-right">${model.output_cost_per_1k.toFixed(5)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>

...
export function FrequencyCounter() {
  ...
  const { authorizations, settlements, avgCostPerAction, totalCostUsd } = frequencyStats;

  // Signed authorizations = all per-chunk billing events (extraction + review variants)
  const signedAuths = authorizations;

  // Tempo on-chain tx = events with a real settlementTxHash
  const tempoTxCount = ledgerEvents.filter((e) => e.settlementTxHash != null).length;
  ...
  // Threshold check: 50+ onchain events at avg ≤ $0.01

...
export const metadata: Metadata = {
  ...
  description:
    'P402 Meter: why per-token AI billing requires Tempo mainnet, how MPP makes it programmable, and what three industry demos prove together.',
};
      ...
      <div className="max-w-4xl mx-auto px-6 py-16 flex flex-col gap-20">
        ...
        <section className="flex flex-col gap-6">
          ...
          <p className="text-base font-mono text-neutral-400 max-w-2xl leading-relaxed">
            P402 Meter is the proof that the problem is solved. Three industries, one settlement
        ...
        {/* ── Three industries ──────────────────────────────────────────────── */}
        <section className="flex flex-col gap-6">
          <SectionLabel number="03" label="Three Industries, One Infrastructure" />
          <h2 className="text-2xl font-bold uppercase tracking-tight">
            Intentionally different workflows. Identical settlement layer.
          </h2>
          <p className="font-mono text-neutral-400 leading-relaxed">
            Each demo was chosen to stress a different dimension of the infrastructure:
            clinical governance, transactional routing, and multimodal volume.
            The router handles all three without modification.
          </p>
          ...
          <div className="flex flex-col gap-4">
            ...
            <IndustryCard
              ...
              status="coming-soon"
              facet="Routing decisions visible. Different documents routed to different model tiers. Budget cap per matter. Per-matter cost breakdown."
            ...
            <IndustryCard
              ...
              status="coming-soon"
              facet="Multimodal volume economics. Fraud signal triggers specialist escalation under MPP escrow. Per-applicant cost at sub-penny scale."
        ...
        <section className="flex flex-col gap-6">
          <SectionLabel number="04" label="What All Three Prove Together" />
          <h2 className="text-2xl font-bold uppercase tracking-tight">
            The infrastructure is the product. The industry is the surface.
          </h2>
          <p className="font-mono text-neutral-400 leading-relaxed">
            Each demo in isolation proves per-token billing works in one domain. The three together
            prove something larger: P402 is industry-agnostic AI cost infrastructure.
          </p>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <ProofCard
              claim="Same router, three regulatory regimes"
              detail="URAC (clinical), ABA (legal), HUD (housing). Different audit artifacts required. Same P402 session model, budget cap, and approval gate satisfies all three. The governance layer is not domain-specific."
            />
            <ProofCard
              claim="Per-token economics are universal"
              detail="Healthcare: $0.0000006/token. Legal: routing by document complexity. Real estate: per-applicant at scale. The token-level pricing model applies everywhere AI reasons over documents."
            />
            <ProofCard
              claim="Specialist escalation is a real workflow pattern"
              detail="In all three demos, complex cases trigger A2A escalation: specialist agent, budget envelope, deliverable hash recorded onchain. This is not a demo gimmick — it mirrors how real-world workflows route hard cases."
            />
            <ProofCard
              claim="The audit trail is load-bearing in every industry"
              detail="Clinical AI must explain decisions for URAC audit. Legal AI must maintain records for ABA ethics. Tenant screening AI must demonstrate consistent criteria for HUD. The P402 ledger provides all three."
            />
          </div>

          <div className="border-2 border-primary p-5 flex flex-col gap-3">
            <div className="text-[10px] font-mono text-primary uppercase tracking-wider">
              The common denominator across all three industries
            </div>
            <p className="text-sm font-bold text-neutral-50 leading-relaxed">
              Every high-stakes AI workflow has the same gap: no metered cost visibility,
              no per-action audit trail, no sub-cent settlement rail.
              P402 fills it once. The demos prove it three times in three
...
function IndustryCard({
  ...
  buyer, regulatory, href, deepDiveHref,
}: {
  number: string; industry: string; workflow: string; status: 'live' | 'coming-soon';
  facet: string; unitEcon: string; vsBaseline: string; buyer: string;
  regulatory: string; href: string; deepDiveHref: string;
}) {
  const isLive = status === 'live';
  return (
    <div className={`border-2 p-5 flex flex-col gap-3 ${isLive ? 'border-primary' : 'border-neutral-700'}`}>
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className={`text-[10px] font-mono font-bold ${isLive ? 'text-primary' : 'text-neutral-600'}`}>{number}</span>
            <span className="text-[9px] font-mono text-neutral-500 uppercase tracking-wider">{industry}</span>
            {isLive
              ? <span className="border border-success text-success text-[9px] font-mono px-1.5 py-0.5 uppercase">Live</span>
              : <span className="border border-neutral-700 text-neutral-600 text-[9px] font-mono px-1.5 py-0.5 uppercase">Soon</span>
            }
          </div>
          <h3 className="text-base font-bold uppercase tracking-tight">{workflow}</h3>
        </div>
        <div className="flex flex-col items-end">
          <span className={`text-lg font-bold font-mono tabular-nums ${isLive ? 'text-primary' : 'text-neutral-500'}`}>{unitEcon}</span>
      ...
      <div className="flex gap-3 flex-wrap mt-1">
        {isLive ? (
          <Link href={href} className="btn btn-primary text-xs px-4 py-1.5">Run Demo →</Link>
        ) : (

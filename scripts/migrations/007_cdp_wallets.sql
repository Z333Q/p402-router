-- Migration 007: CDP Wallet Registry + Session Wallet Source
-- ============================================================
...
-- rows or queries are broken.
--
-- Run with:
--   psql $DATABASE_URL -f scripts/migrations/007_cdp_wallets.sql
...
COMMENT ON COLUMN agent_sessions.wallet_source   IS 'cdp | eoa | null — how the session wallet was provisioned';
...
CREATE TABLE IF NOT EXISTS cdp_wallet_registry (
    ...
    session_id       VARCHAR(100) REFERENCES agent_sessions (session_token) ON DELETE SET NULL,

...
export default function NavConnectButton() {
    const { state: authState, walletAddress, isLoading } = useAuthState();

    if (isLoading) return null;

    // Google user with no wallet — show activation prompt
    ...
    return (
        <ConnectButton.Custom>
            {({
                ...
                // Wagmi not yet connected but we know the user has a wallet (CDP email or
                // previously linked external wallet) — show a static address chip so we
                // never render the "Connect Wallet" RainbowKit trigger in the nav.
                if (!connected) {
                    if (!walletAddress) return null;
                    const short = `${walletAddress.slice(0, 6)}…${walletAddress.slice(-4)}`;
                    return (
                        <div className="btn btn-secondary text-xs !py-1.5 !px-3 font-mono">
                            {short}
                        </div>
                    );
                }
